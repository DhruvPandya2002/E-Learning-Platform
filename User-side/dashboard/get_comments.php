...
$result = mysqli_query($conn, "SELECT * FROM `premium_comment` WHERE `id` = '$_SESSION[premium_id]' and `premium_course_id` = '$premium_course_id' and `flag` = 0 and `parent_id` = '$reply_flag' ORDER BY `date` DESC");
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    <div class="flex flex-col items-start">
        ...
        <button class="reply_btn ml-1 mt-3 bg-[#DCE1F9] px-4 py-2 rounded-full font-medium text-[#30559E]" comment-id="'.$row['id'].'" onclick="setParentId('.$row['id'].')" id="replyComment">Reply</button>';
            ...
            $replied_result = mysqli_query($conn, "SELECT * FROM `comment` WHERE `content_id` = '$_SESSION[content_id]' and `course_id` = '$course_id' and `flag` = 0 and `parent_id` = '$reply_flag' ORDER BY `date` DESC");
            ...
            while ($replied_row = mysqli_fetch_assoc($replied_result)) {
                echo '<div class="flex mt-3 ml-8">
                    ...
                    all_comment_letters_images($replied_row['sender']);
                    echo '</div>
                    <div class="flex flex-col items-start">
